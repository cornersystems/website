...
- [ ] **CRM full-width layout** — remove the fixed `max-width` and excessive horizontal padding on CRM tab content so pages use the full available width. Current side margins make tabs feel cramped, especially on wider screens. Check `.crm-content`, `.crm-table-wrap`, and any per-tab width constraints in `src/styles.css`.
